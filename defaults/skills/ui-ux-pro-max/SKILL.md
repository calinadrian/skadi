...
On Skadi or another Node-based host where none of those Python commands exists, use the bundled dependency-free fallback with the same query shape:

```bash
node "${CLAUDE_PLUGIN_ROOT}/.claude/skills/ui-ux-pro-max/scripts/search.mjs" "<query>" --domain <domain>
```

The Node fallback searches the same bundled CSV catalogs and supports domain, stack, design-system, JSON, persistence, page, and force modes. Python remains the canonical engine when available because it also runs the full reasoning and calibration contracts.
